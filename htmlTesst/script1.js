document.addEventListener("DOMContentLoaded", function () {
  ...
  addToCartButtons.forEach((button) => {
    button.addEventListener("click", function () {
      ...
      const product = {
        id: this.dataset.name, // Sử dụng tên làm ID
      ...
      // Kiểm tra nếu sản phẩm đã tồn tại, không cộng dồn số lượng vào tổng số sản phẩm
      ...
      // Cập nhật số sản phẩm trong thông báo
  ...
  // Cập nhật số lượng hiển thị trên thông báo giỏ hàng
  function updateCartNotification() {
    let cart = JSON.parse(localStorage.getItem("cart")) || [];
    let totalProducts = cart.length; // Chỉ tính số lượng sản phẩm khác nhau
    ...
    if (notificationIcon) {
      ...
      // Luôn hiển thị số lượng, ngay cả khi giỏ hàng trống
      notificationIcon.style.setProperty("--count", `"${totalProducts}"`);
    }
  }
  ...
  if (document.getElementById("cartItems")) {
    ...
    function renderCart() {
      ...
      updateSelectedTotal(); // Cập nhật tổng thanh toán
    }
    ...
    cartTableBody.addEventListener("input", function (event) {
      if (event.target.classList.contains("quantity-input")) {
        ...
        if (newQuantity < 1) {
          ...
          event.target.value = cart[index].quantity; // Giữ nguyên số lượng cũ
          return;
        }
        cart[index].quantity = newQuantity;
        localStorage.setItem("cart", JSON.stringify(cart));
        renderCart();
        updateSelectedTotal(); // Cập nhật tổng số tiền sau khi đổi số lượng
        localStorage.setItem("cart", JSON.stringify(cart));
        renderCart();
      }
    });
