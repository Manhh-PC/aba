    ...
    async function loadUserInfo() {
      try {
        ...
        if (user && user.email) {
          ...
          // Kiểm tra xem userName2 có tồn tại không trước khi gán giá trị
          const userName2Element = document.getElementById("userName2");
          if (userName2Element) {
            userName2Element.textContent = user.name;
          }
      ...
      if (logoutBtn) {
        logoutBtn.addEventListener("click", async () => {
          try {
            ...
            alert("Đã có lỗi xảy ra khi đăng xuất! Kiểm tra Console để biết chi tiết.");
          }
        });
      }
    }
